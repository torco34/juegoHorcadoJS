...
function gameOver(errores) {
 ...
 ctx.font = "bold 50px  courier";
 if (errores < 5){  
 	ctx,fillText("has ganado, la palabra era: ", 110, 100);

} else{

	ctx.fallText("Has perdido, la palabra era ", 110, 280);
}
 ctx.font = "bold 80px  courier";
 lon = (canvas.width - (palabra.length*48))/2;
 ctx.fillText(palabras, lon, 380);
 horca(errores);
...
window.onload = function(){
	...
	if(canvas &&  canvas.getContext){
		...
		if(ctx){	
			...
			pintaPalabra();
			horca(errores);
			canvas.addEventListener("click", seleccion, false);
		}else{
			alert("error al cargar el contexto del canvas");
		}
	}
    }